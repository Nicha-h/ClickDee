...
- `src/components/` — shared/reusable components (`navbar.tsx`, `Topbar.tsx`, `notificationPanel.tsx`). File casing is inconsistent (`Topbar.tsx` vs `navbar.tsx`); match whichever a sibling file already does rather than establishing a new casing. `notification.tsx` (singular, no "Panel") is a separate, unused dead placeholder (`return <div></div>`, not imported anywhere) — don't confuse it with `notificationPanel.tsx`, which is the real, wired-up component.
...
- `pages/campaign.tsx` (the campaign list) is the one page that merges the static `data/campaigns.ts` demo data with real campaigns fetched from `getApiCampaigns` — `toCampaignItem` adapts a real `Campaign` (`name/caption/status/budget/startDate/endDate`) into the richer mock `CampaignItem` shape, defaulting every analytics field (reach/clicks/roi/creatives/...) to empty since the real model doesn't have them yet; adapted items carry `source: 'api'` so the page can hide actions the mock-only pages don't support. The backend now *does* have `PATCH`/`DELETE /api/campaigns/:id` (`patchApiCampaignsId`/`deleteApiCampaignsId` in the generated client) — added for the AI confirm-flow (see `pages/ai.tsx` note below), not yet wired into any human-facing edit/delete UI on this page. `campaignReport.tsx`/`campaignEdit.tsx` are **not** part of this — they still only read the static mock array by id and gracefully 404 ("ไม่พบแคมเปญนี้") for a real campaign's id; don't assume they work for `source: 'api'` items without checking first.
